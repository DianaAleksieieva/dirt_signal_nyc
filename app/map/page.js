"use client";

import Map311Client from "./Map311Client";

export default function MapPage() {
  return (
    <div className="min-h-screen bg-eco-beige text-eco-text pt-0 px-6 pb-6">


      <div className="flex flex-col lg:flex-row gap-6">
        {/* MAP SECTION */}
        <div className="lg:w-4/5 w-full">
          {/* BUTTON BAR ABOVE MAP */}
          <div className="mb-4 flex flex-wrap gap-3">
            <button className="px-4 py-2 rounded-lg bg-eco-green-soft text-eco-green-dark font-medium shadow-sm hover:bg-eco-green-dark hover:text-white transition">
              🗑️ 311 Complaints
            </button>

            <button className="px-4 py-2 rounded-lg bg-eco-green-soft text-eco-green-dark font-medium shadow-sm hover:bg-eco-green-dark hover:text-white transition">
              🗺️ Census Tracts
            </button>

            <button className="px-4 py-2 rounded-lg bg-eco-green-soft text-eco-green-dark font-medium shadow-sm hover:bg-eco-green-dark hover:text-white transition">
              🚶 Pedestrian Flow
            </button>

            <button className="px-4 py-2 rounded-lg bg-eco-green-soft text-eco-green-dark font-medium shadow-sm hover:bg-eco-green-dark hover:text-white transition">
              🗑️ DSNY Bins
            </button>

            <button className="px-4 py-2 rounded-lg bg-eco-green-soft text-eco-green-dark font-medium shadow-sm hover:bg-eco-green-dark hover:text-white transition">
              🔥 Hotspots
            </button>
          </div>

          {/* THE MAP */}
          <div className="rounded-xl h-[500px]">
            <Map311Client />
          </div>
        </div>

        {/* DESCRIPTION SECTION */}
        <div className="lg:w-1/4 w-full bg-eco-green-soft p-5 rounded-xl shadow-sm space-y-5">
          <h2 className="text-xl font-semibold text-eco-green-dark">
            NYC Cleanliness
          </h2>

          <div className="space-y-1">
            <p className="text-eco-green-dark font-semibold text-lg">
              📍 311 Trash Complaints
            </p>
            <p className="text-2xl font-bold text-eco-green-dark">42,310</p>
            <p className="text-sm text-eco-text-dark/70">
              Total complaints recorded in 2024 (placeholder).
            </p>
          </div>

          <div className="space-y-1">
            <p className="text-eco-green-dark font-semibold">
              🏙️ Borough Cleanliness Index
            </p>
            <ul className="text-sm text-eco-text-dark/80 list-disc ml-5 space-y-1">
              <li>
                <span className="font-semibold">Manhattan:</span> Highest
                density of complaints
              </li>
              <li>
                <span className="font-semibold">Staten Island:</span> Cleanest
                overall
              </li>
              <li>
                <span className="font-semibold">Bronx:</span> High hotspot
                frequency
              </li>
            </ul>
          </div>

          <p className="text-sm text-eco-text-dark leading-relaxed">
            These metrics summarize trash-related 311 reports, population
            density, foot-traffic behavior, and availability of public waste
            bins. Data will auto-update when real datasets are connected.
          </p>
        </div>
      </div>
    </div>
  );
}
